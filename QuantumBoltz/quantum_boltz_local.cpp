...
void print_to_file(vector <int>& time,vector <double>& occ_number,vector <double>& kin_energy, vector <double> & pot_energy,std::string filename,int interval,int it,int & first_time);
...
public:
	...
	long numOfFreq_;
    int size_; //=2
    double omega_ph_=0.2;
    double lambda_ = 2.5; //0.0; //lambda should be 0.5 or 1 or 2.5
    double g_el_ph_ =sqrt(lambda_*omega_ph_);   
    vector<GF> Gk_;
    GF Sigma_,Gloc_,Bath_,G1_,G2_,Delta_,Floc_,Phonon_,Sigma_Phonon_,Bath_fermion_bump_,Gweiss_,Sigma_plus_Delta_;
...
void updateSE1(double U, const GF &GA, GF &locSEA, fft_solver &solver, GF & Bath, GF & Sigma_Phonon, GF & Bath_fermion_bump)
{
...
void get_Sigma_ph(double g_el_ph,const GF & GA,const GF & Phonon,GF & Sigma_ph,fft_solver &solver){
cplx ii(0.0,1.0);
...
tSigma_ph.smul(g_el_ph*g_el_ph);
...
//ForceImag(Sigma_ph); //do I need this?
}
...
public:
    ...
    void sigma2_bold(lattice_func &latt,double U){
        latt.get_gloc();
        updateSE1(U,latt.Gloc_,latt.Sigma_,solver_,latt.Bath_,latt.Sigma_Phonon_,latt.Bath_fermion_bump_);
    }
    void sigma2_ipt(lattice_func &latt,double U){
	...
	get_Sigma_ph(latt.g_el_ph_,latt.Gloc_,latt.Phonon_,latt.Sigma_Phonon_,solver_);	
        updateSE1(U,Gweiss,latt.Sigma_,solver_,latt.Bath_,latt.Sigma_Phonon_,latt.Bath_fermion_bump_);
...
double dmft_iteration(int iter,lattice_func &latt,sigma_solver &imp,int ipt,double U,int ret,double mixing,int tstp,string flag){
    double err; 
    GF G_old=latt.Gloc_;
   
    if(ipt){
        imp.sigma2_ipt(latt,U);
    }else{
        imp.sigma2_bold(latt,U);
    }
    if(ret==1) {
	latt.get_hartree(U);	
...
void dFdt(lattice_func &latt_new,lattice_func &latt,sigma_solver &imp,double U,double errmax,int Niter,int ipt,std::string flag,double mixing, int ret,int tstp){
    double err=10.0;
    int i=0;
   
   cout<<"U = "<<U<<endl;
    while(i<Niter && err>errmax){
      err=dmft_iteration(i,latt,imp,ipt,U,ret,mixing,tstp,flag);
...
int main(int argc, char** argv){
  ...
  int it =0, ret=1,interval=100,first_time=1; 
  int tmp_int;
  double err_F;
  ...
  int ipt;
   int outputf,param;
	...
	string direct_out="/nfs/tfkp00/picanoan/Desktop/C++/codes/boltzmann/standalone_ness_mio/param_"+std::to_string(param)+"/";
	...
	vector <double> occ_number(nt+1,0.0), kin_energy(nt+1,0.0), pot_energy(nt+1,0.0) ;
        ...
        block_name.str("");
        block_name.clear();
        block_name<<"_U_"<<U_i<<"_beta_"<<beta<<"_h_"<<h<<"_T_Vt_"<<T_Vt<<"_Amp_Vt_"<<Amp_Vt<<"_tstp_max_"<<nt;
	...
	GF_err=dmft_iteration(i,latt,imp,ipt,U,ret,mixing,it,"R1_");       
	...
	PotEnergy(latt.Gloc_,latt.Sigma_,R_Epot,0);
	...
	/*
        os.str("");
        os.clear();
        os<<direct_out<<"temp_occupation_plus_energy_equilibrium_fcutoff"<<freq_cutoff<<"_"<<block_name.str()<<".out";
        filename= os.str();
        print_to_file_equilibrium(time,occ_number,kin_energy,pot_energy,filename,interval,it,first_time,numOfFreq);//print in append mode
	*/
         os.str("");
        os.clear();
	...
	set_fermion_bath_bump(latt.Bath_fermion_bump_,V);
        ...
        dFdt(lk2,ltmp,imp,U,lerr,NiterR,ipt,"R2_",mixing,ret,it);
        smul_les_F(lk2,mult);
        ...
        set_fermion_bath_bump(latt.Bath_fermion_bump_,V);
        U=sin_ramp(t0+0.5*h,tramp,U_i,U_f);
        dFdt(lk3,ltmp,imp,U,lerr,NiterR,ipt,"R3_",mixing,ret,it);
        smul_les_F(lk3,mult);
	...
	//I calculate k4:
        ...
        dFdt(lk4,ltmp,imp,U,lerr,NiterR,ipt,"R4_",mixing,ret,it);
        smul_les_F(lk4,mult);
...
*/        
        ...
        U=sin_ramp(t0+h,tramp,U_i,U_f);
               ...
               while(i<NiterR && GF_err>lerr){
	  GF_err=dmft_iteration(i,latt,imp,ipt,U,ret,mixing,it,"R5_");
	...
	cout << "EkinA = " << R_Ekin(0,0).real() << " " << R_Ekin(0,0).imag()<<endl;
	//Calculate Epot
	PotEnergy(latt.Gloc_,latt.Sigma_,R_Epot,it);
...
void print_to_file(vector <int>& time,vector <double>& occ_number,vector <double>& kin_energy,vector <double> & pot_energy,std::string filename,int interval,int it, int & first_time){
	...
	if(first_time==1){
	 outputFile << "# time" << "\t" << "Occupation" << "\t"<< "Kin_energy" <<"\t"<<"Pot Energy"<<std::endl;
	start=0;
	stop=1;
	}
